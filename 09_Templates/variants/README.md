...
Context-specific **source material** for fork-time template specialization (issue #12, option 2). During [onboard-owner](../../10_Agents/skills/setup/onboard-owner/SKILL.md)'s context-specialization step, a **work**-context answer rewrites the shipped periodic templates in `09_Templates/` **in place** from the matching file here — template paths never change, so everything that resolves templates by stable name (`daily_note.py`, the daily-log and periodic-review skills, snippet generation) keeps working untouched.
...
- [onboard-owner](../../10_Agents/skills/setup/onboard-owner/SKILL.md) — The specialization step
