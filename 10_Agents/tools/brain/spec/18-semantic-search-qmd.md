...
- **`--stdin-json`** — the harness/API ingestion interface. Stdin uses the versioned envelope below. `schemaVersion` is the integer `1` (not a boolean), `model` is a non-empty string, and `notes` is a non-empty object; these are the only top-level keys. Every note entry has exactly `hash` and `vector`: `hash` is the lowercase SHA-256 hex digest of the full normalized source text (§3, frontmatter included) **that the adapter actually embedded**; `vector` is a non-empty array of finite numbers (booleans excluded). All vectors have the same dimension. Paths use §2 normalization; unknown paths and duplicate normalized paths fail the whole call. The unversioned `{model, vectors}` envelope is rejected with a migration hint: brain cannot safely infer which source revision produced an old vector.

  ```json
  {"schemaVersion": 1, "model": "example-model", "notes": {"note.md": {"hash": "<64 lowercase hexadecimal characters>", "vector": [0.1, 0.2]}}}
  ```

  Brain captures each source once and compares the supplied digest with that snapshot. A missing/invalid digest or any mismatch is an operational error (stderr, exit 1, **sidecar untouched**, including otherwise valid entries in the same request). The adapter must re-read and re-embed changed notes; it must never repair a mismatch by hashing a newer file without recomputing its vector. Entries for restricted notes are **accepted** per §18.1. Entries whose source cannot be read or decoded (§3) are skipped with a stderr notice — there is no readable source to compare. An edit after capture makes the persisted source digest stale on the next query; it cannot certify the newer text as embedded.

  **Partial-update semantics:** when the incoming model and vector length match the existing store's model/dimension, entries merge over it using their verified **supplied source hashes**; unmentioned notes keep their entries. A different model or dimension replaces the store wholesale with a stderr notice. Output: `{"dim", "model", "path", "skippedUnreadable", "stored"}` under `--json`, the same facts as text otherwise (`--local` emits `{"dim", "embedded", "model", "path", "total"}`). There is no `skippedRestricted` field; restricted notes remain part of the local embedding universe.

- **`--local [--model NAME]`** — the offline path, and the **one sanctioned optional non-stdlib dependency** in the vault: `sentence-transformers`, imported lazily inside this feature only, never at module import time, never required. When the import (or model load) fails, `embed --local` exits 1 with a message naming the three alternatives (install the optional package, pipe vectors via `--stdin-json`, or use keyword search) — a clean degradation message, never a traceback. When available: capture each note once and use that same normalized text for parsing, its digest, and model input; a concurrent edit makes the stored snapshot stale on a subsequent query. Embed every readable note whose entry is missing or stale (restricted notes included, §18.1; incremental by hash; fresh entries untouched), under the model named by `--model`, else the store's recorded `model`, else the default constant (`EMBED_LOCAL_MODEL_DEFAULT`); a model different from the store's replaces the store wholesale, as above. When there is nothing to embed **and** nothing retained (an empty vault, or every note unreadable), the command reports and exits 0 **without writing** — a `{"dim": null}` store would violate the §18.1 shape.
...
Ranks whole notes by a hybrid of vector similarity and keyword match. Metadata, privacy classification, keyword hits, and freshness hashes use the same captured note snapshots (§9), including when ranking degrades to keyword search. `--tag` filters (effective-tag semantics, §9) restrict the note universe for **both** components. Exit `0` on success — including every degraded case; `1` only for operational errors (malformed `--query-vector` input).
...
- **Output** (semantic mode): JSON — an array of `{"keywordHits": int, "path", "restricted": bool, "privacy": "public|private|unknown", "explicitRestricted": bool, "score": float, "semanticScore": float|null, "title"}` rows in rank order (`semanticScore` is the rounded `sem(n)`, `null` when the note has no fresh vector; `restricted` conservatively marks effective private or unknown classification, `privacy` names that classification, and `explicitRestricted` reports the original tag — the same §9 provenance treatment as keyword rows); human — one `score  path  (title)` line per row, with a trailing `  [restricted]` marker on restricted rows. This intentionally ranks *notes* where plain `search` lists *hits*; the degraded mode keeps plain `search`'s shape so a vectorless vault still gets exactly the §9 contract.
