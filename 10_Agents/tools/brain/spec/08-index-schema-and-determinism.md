...
The committed index must be a **pure function of tracked file contents** — a fresh CI clone rebuild must be byte-identical to the committed copy. The commit hook materializes the complete staged tree, including skip-worktree paths; it never combines staged path names with unstaged source bytes. Runtime queries may still inspect working-tree content. Hence the index corpus in §2, plus:
...
- **Merge driver (M8.6, issue #25):** `.gitattributes` additionally marks the three committed generated files — `10_Agents/tools/brain/vault-index.json`, `.vscode/second-brain.code-snippets`, and the compiled `00_Meta/BOOTSTRAP.md` (§28) — with `merge=regenerate`. The driver is defined per clone as `git config merge.regenerate.driver true` (the `true` command exits 0 leaving `%A` = ours): merges of generated content resolve keep-ours, and **correctness comes from regeneration, not resolution** — the pre-commit hook regenerates and validates against a private staged-tree snapshot. Because Git computes automatic merge trees before `pre-merge-commit`, that hook stages fresh generated output and pauses an automatic merge when the tree changes; `git commit` finishes it through normal validation. `post-merge` only checks committed bytes. `pre-push` verifies outgoing committed tips, and automatic push/PR CI checks freshness without writing. Full tests and generated-file repair remain separately dispatched; requiring the automatic CI status for GitHub merges is a branch-protection setting. Clones without the driver configured degrade to a normal conflict plus the documented fallback recipe (`10_Agents/solutions/vault-tooling/index-merge-conflicts.md`). Any future committed generated file adopts the same attribute.
...
### Derived classification and public-only synthesis

`effective_privacy(index)` computes `public`, `private`, or `unknown` from one snapshot. Actual derivation edges are explicitly declared `privacy-sources` on any note (including AYMT/Home's generated manifests) and canonical Area `## Active Projects` links; ordinary bare links never propagate classification. Private or unknown sources propagate through multiple derivation steps. Missing/unreadable sources and missing/invalid generated manifests are unknown; cycles cannot establish public provenance by referring only to one another. No Git baseline, clock, or environment is consulted.

Ordinary indexing and commits never fail because source or derived content is private or unknown. `reduce_restricted` retains all records; its index representation adds `restricted/private` to effectively private derived records and applies normal body reduction. Derived records carry `frontmatter.effective-privacy` to distinguish unknown provenance. This describes the effective index view without hand-editing a generated source file.

`public_only_index(index)` explicitly excludes private and unknown notes before content is emitted, filters backlinks to admitted notes, and recomputes link counts. It leaves the local index intact. Missing provenance excludes a derived record only in this view. Ordinary local reads and search retain private content; §9 defines the public-only query/read flags.
