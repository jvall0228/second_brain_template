...
`brain bootstrap` prints the rendered bytes; `--check` prints `<path>: fresh|stale|missing (<size> / <budget> bytes)` and exits 1 when the committed bytes differ, the file is absent, or the size exceeds `BOOTSTRAP_TOTAL_BUDGET` (the same 32 KiB cap as the six sources combined). The rendered file is **not** guaranteed smaller than its sources — dropping frontmatter saves less than link rewriting (`x.md` → `../01_Profile/x.md`), the generated header, and the per-section source lines add — so the cap is enforced on the rendered bytes independently of the §14 per-source budgets: `--write` **refuses** an over-budget render (prints `refused … OVER BUDGET` to stderr, leaves any committed file untouched, exits 1); otherwise it writes only when the bytes differ and prints `written` or `unchanged`. JSON carries `{budget, fresh, overBudget, path, sizeBytes, sources: [{path, restricted, sizeBytes, title, updated}], state[, write]}`. A missing or undecodable source is an error (exit 1) — the file is never written partially.

Ownership: `--write` is the only writer; hand edits are foreign. The pre-commit hook regenerates and validates against a private copy of the complete staged tree, preserving unrelated unstaged source edits. Generated results are staged only after validation succeeds. The `regenerate` merge driver keeps ours for generated conflicts; `pre-merge-commit` stages fresh output and pauses an automatic merge if regeneration changes its tree, so `git commit` records the validated result. `post-merge`, `pre-push`, and automatic push/PR CI check committed consistency without writing. Full tests and generated-file repair run only through their separate manual workflows. `brain context` appends the compiled file's size and state after the totals line. The staged `BOOTSTRAP.md` is always a function of the staged source bytes, including skip-worktree paths.
...
**Privacy classification.** Bootstrap retains complete private source bodies for internal context. If any source carries `restricted/private`, compiled frontmatter inherits that tag, and JSON source rows record `restricted`. Classification does not prevent compilation or local use. A bootstrap containing private sources is not suitable for a fresh public-only synthesis context.
