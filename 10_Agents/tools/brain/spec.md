...
- **Kept:** path (the `notes` key), `title`, `frontmatter` (including `tags` — consumers must be able to see *why* the record is reduced), `updated`, `sizeBytes`, `frontmatterErrors`, `links`, and `backlinks`. Links and backlinks stay so restricted notes remain discoverable and the §10.2 containment check has structure to work with.
- **Dropped (emptied, not omitted — §8.1's every-field-present shape holds, so no `schemaVersion` bump):** `headings: []` and `bodyTags: []` — the body-derived fields the index would otherwise publish.
