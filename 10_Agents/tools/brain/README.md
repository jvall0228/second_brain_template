...
Use `--shared-only` for shared notes without selecting a machine environment. It excludes environment subtrees and conflicts with `--env`.

For synthesis using public notes only, select the [policy-only context route](../../../00_Meta/restricted-private.md#public-only-synthesis) before personal bootstrap. Within that isolated context, use `search --public-only`, `show --public-only`, or `read NOTE... --public-only`. Private and unknown derivations are excluded before source output; a later filtered query cannot remove earlier personal context. Internal generation may use private content. These commands do not send messages.
...
| `list` | Note paths; filters: `--dir PREFIX`, `--tag TAG` (repeatable; `type/*` matches a namespace), `--type X`. Query rows in `--json` (here and in `search`/`recent`/`tasks`/semantic) carry effective `privacy` classification, a conservative `restricted` boolean, and the explicit source tag separately (spec §9/§17/§18) |
...
| `home` | Deterministic local Home (spec §24) built from structured AYMT plus eligible tracked tasks, including private work, Inbox, active work, reviews, current state, health, environment metadata, and navigation. Preview/JSON/check are zero-write; explicit `--write` updates only recognized generated `00_Meta/HOME.md`. Optional `--github-input PATH\|-` affects only AYMT actions; brain never invokes GitHub or the network |
...
| `show <note>` | The full index record for one note; `--public-only` applies effective source classification before output |
| `read <note>...` | Full source text and content hashes from captured snapshots; `--public-only` refuses a selection containing private or unknown derivations before emitting any source text |
...
| `curate` | Re-review signals: expired / missing / over-cap `expires:`, oversized, stale (backlink-weighted), orphans, unreferenced assets, and distill candidates (backlinked Journal/solution notes not yet reshaped into a zettel, spec §14). `--check-urls` adds network URL probes (opt-in; never pre-commit) |
...
| `triage-archive <report>` | Roll an applied `02_Inbox/` triage report into the month's `07_Archives/inbox/YYYY-MM-triage-log.md` (spec §29.1): document H1 dropped, headings demoted, relative links rebased, identity marker (path plus digest), append-then-delete that a retry finishes without duplicating, and a `conflict` (nothing touched, exit 1) when the same path was archived with different bytes — `--revise` appends a distinct revision instead; a private or unknown report moves as its own classified note. Source removal authenticates the captured file identity and parent chain and preserves concurrent replacements with recovery evidence. All four plan-and-write commands (`triage-archive`, `trace`, `gap`, `accepted`) serialize on `.second-brain/write.lock` and compare-and-swap every write, so concurrent runs never lose a record. Preview by default; `--write` applies and validates |
...
`vault-index.json` is committed so agents can read structured vault data **without running anything** — per note: frontmatter, headings, links with resolution state, backlinks, body tags. Check `schemaVersion` before consuming. Commit and merge hooks regenerate from staged sources and validate that snapshot while preserving unstaged source edits. Automatic read-only CI checks committed consistency; manual validation and repair remain a separate workflow.
...
The runner is the canonical entrypoint for every tool suite (see [README](../README.md)); plain `unittest discover -s 10_Agents/tools/brain/tests` still works for this suite alone. Live-corpus checks use explicit shared scope; do not export a machine environment for fixture suites. Fixture mini-vaults under any `10_Agents/tools/*/tests/` tree are excluded from the real corpus by design (spec §2).
