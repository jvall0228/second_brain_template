...
- Standing exception: a live `configure-notifications` session may write only `.second-brain/environments/<slug>/{notifications.json,notification-state.json,notification-output/}`, authenticated ignored `.{notifications.json,notification-state.json}.{notify-claim,migrate-{new,old}-*}` generation/recovery evidence, or a new, explicitly approved file below the system temporary directory. The owner must name and acknowledge a private destination and opt in each category; this exception never permits real-provider delivery, inbound actions, restricted content, or Outbox shipping
