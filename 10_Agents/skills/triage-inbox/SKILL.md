...
4. **Classify each note:**
   - **Autonomous log rows first.** Two capture shapes are produced by `brain` for autonomous runs and have a mechanical destination, not a PARA one: a gap capture (`author: brain`, slug `vault-answer-gap`) ingests with `brain gap --ingest <capture> --write`, which appends its row to [vault-answer-gaps](../../docs/vault-answer-gaps.md) once (identity marker) and deletes the capture; a `restricted/private` gap capture is refused unless the owner declassifies it with `--declassify`, and otherwise stays in the Inbox as the owner's open decision. A spec-retrospective report's `## Accepted proposals` table ingests with `brain accepted --ingest <report> --write`, which appends the rows not yet present to [accepted-proposals](../../docs/accepted-proposals.md) and leaves the report to be triaged as a report. Both commands are idempotent, so re-running a report adds nothing.
