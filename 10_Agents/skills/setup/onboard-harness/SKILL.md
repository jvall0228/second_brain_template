...
1. **Skills (symlink-first).** Create only the links/copies approved in the exact global preview from the harness's user-level skills discovery path back to the canonical folders:
   ...
   - **Copilot is the exception — no symlinks:** its CLI ignores symlinked skills and does not reliably discover `~/.agents/skills/`. Register the vault's generated flat adapter directory instead (`copilot skill add <vault>/.agents/skills` — real directories, one immediate child per catalogued skill whatever its canonical depth, so the grouped setup skills are discovered too; never `10_Agents/skills/`, whose grouped skills are not immediate children; uninstall via `copilot skill remove`), or copy each catalogued skill's canonical folder, named by skill, into `~/.copilot/skills/` with hashes recorded for drift re-sync — see [wiring](../../../harnesses/copilot/wiring.md).
...
4. **Harness memory adapter.** Add a marker-delimited block to the harness's **documented user-level instruction surface**, creating that file only when the harness supports a file there. The block points to `~/.agents/second-brain/AGENTS.md`, **not directly to any vault**. Prefer native include/import syntax when verified for that harness; otherwise use a plain instruction telling the agent to read the shared registry when owner context is relevant. Example native include:
   ...
   <!-- END second-brain registration -->
   ```
   The surface and syntax come from the harness wiring doc. Everything outside the markers is the user's own content: never touch it. Re-running replaces one managed block; collapse duplicates. Because all adopted vaults share the registry, onboarding another vault does **not** add another harness-level block.
