...
> **Re-verified in depth 2026-08-11** — same day as the original M6 pass, to ground the P0 promotion ([[07_Archives/inbox/2026-08-11-copilot-p0-plan|plan + review log]]). Five parallel research agents read the live GitHub and VS Code documentation pages directly (not summarization passes); load-bearing claims quote their sources. This section absorbed the standalone deep-dive note (formerly `06_Resources/copilot-harness-deep-dive.md`), replacing the M6 pass it superseded.
...
#### Corrections to the M6 baseline
...
5. **The Copilot CLI has a real `@`-include mechanism** (the M6 pass said "no import mechanism"): in `.github/copilot-instructions.md`, `AGENTS.md`, or `CLAUDE.md`, `@relative/path` includes another file, recursively. Not expanded in `GEMINI.md` or `*.instructions.md`; absolute paths and `~/` paths are **not** loaded. So the vault's one-line `CLAUDE.md` (`@AGENTS.md`) is genuinely expanded by the CLI — and the CLI "removes duplicate copies of identical" instruction files, so the shim costs nothing there. ([CLI custom instructions](https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/add-custom-instructions))
...
- No documented cross-path precedence for duplicate names; CLI built-ins can "intermittently override" same-named user skills ([#2146](https://github.com/github/copilot-cli/issues/2146)). **Avoid the nine CLI built-in names**: analyze, design, document, fix, investigate, research, security, test, verify. (None of the vault's skills collide.)
