...
- **M6 rescoped to the plugin-library core:** skills adopt the Agent Skills format (folder-per-skill with `SKILL.md`); initial families are capture & triage, periodic reviews, vault maintenance, research → resource, and an onboarding installer (symlink-first into each harness's discovery paths for the primitives it supports, with merge/copy fallbacks; manifest-driven and reversible); P0 adapters then P1, per §8.3.
